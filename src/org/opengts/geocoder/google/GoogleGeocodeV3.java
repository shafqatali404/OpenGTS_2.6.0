    ...
    implements ReverseGeocodeProvider, GeocodeProvider
{
   ...
   // protected static final double MAX_ADDRESS_DISTANCE_KM = 4.5;
    ...
    public GoogleGeocodeV3(String name, String key, RTProperties rtProps)
    {
        ...
        if ((CACHE_MAXIMUM_SIZE > 0L) && (CACHE_MAXIMUM_AGE_MS > 0L) && (AUTO_TRIM_INTERVAL_MS > 0L)) {
            this.rgCache = new ReverseGeocodeCache(CACHE_MAXIMUM_SIZE, CACHE_MAXIMUM_AGE_MS);
            this.rgCache.startAutoTrimThread(AUTO_TRIM_INTERVAL_MS);
        }
    ...
    **/
    public GoogleRoads getGoogleRoads()
    {
        if (this.googleRoads == null) {
            this.googleRoads = new GoogleRoads(this.getName(), null, this.getProperties());
        }
    ...
    public ReverseGeocode getAddressReverseGeocode(GeoPoint gp, String localeStr, boolean cache)
    {
        ...
        if (revGeo != null) {
            ...
            if ((inclSpeedLim == SPEEDLIMIT_MOVING) && !isMoving) {
                ...
                ReverseGeocode speedLimRG = this.getGoogleRoads().getReverseGeocode(gp,localeStr,cache);
                if (speedLimRG != null) {
                    revGeo.setSpeedLimitKPH(speedLimRG.getSpeedLimitKPH());
                }
            }
        ...
        if (status.equals(STATUS_OVER_QUERY_LIMIT)) {
            ...
            failoverTimeoutSec = DateTime.MinuteSeconds(1);  // failover for 1 minute
        } else
        ...
        if (status.equals(STATUS_FORBIDDEN_403)) {
            ...
            failover           = true;
            failoverTimeoutSec = DateTime.MinuteSeconds(5);  // failover for 5 minutes
        } else {
            // -- Unknown error
