    ...
    extends AbstractClientPacketHandler
{
    ...
    public static       String  TK103_KEEPALIVE_ACK         = null;
    ...
    **/
    public byte[] getHandlePacket(byte pktBytes[]) 
    {
        ...
        if (s.startsWith("##")) {
            ...
            // -- create keep-alive ACK packet
            if (!StringTools.isBlank(TK103_KEEPALIVE_ACK)) {
                // -- 1) "LOAD"
                // -- 2) "**,imei:%{IMEI},Q;" : start cached event download
                // -- 3) "**,imei:%{IMEI},S;" : stop cached event download
                String loadAck = StringTools.replace(TK103_KEEPALIVE_ACK, "%{IMEI}", imei);
                Print.logInfo("Keep-Alive ACK: " + loadAck);
                return loadAck.getBytes(); // ACK "Load"
            } else {
                // -- default
                Print.logInfo("Keep-Alive ACK: n/a");
                return null;
            }
        } else
    ...
    **/
    private byte[] parseInsertRecord_TK103_3(String s) // handleCommon
    {
        ...
        // 0----+----1----+----2----+----3----+----4----+----5----+----6-
        // |     ||         |          |    |     |     |       ||>72/54: Miles/Meters? (HEX)
        // |     ||         |          |    |     |     |       |>71/53: Mileage "L"
        // |     ||         |          |    |     |     |>63: GPIO
        // |     ||         |          |    |     |>57/39: Heading
        // |     ||         |          |    |>51/33: Time HHMMSS
        // |     ||         |          |>46/28: Speed Knots (km/h?)
        // |     ||         |>35/17: DDDmm.mmmmH Longitude
        // |     ||>25/7: DDmm.mmmmH Latitude
        // |     |>24/6: A=Valid, V=Invalid
        // |>18/0: Date YYMMDD
        ...
        double   speedKPH   = _speed * KILOMETERS_PER_KNOT; // [2.6.0-B82]
    ...
    **/
    private byte[] parseInsertRecord_VJoy(String s) // handleCommon
    {
        ...
        double speedKPH   = _speed * KILOMETERS_PER_KNOT; // km/h  [2.6.0-B82]
    ...
    **/
        ...
        CellTower servingCell)
    {
        ...
        if (fuelLevel > 0.0) {
        Print.logInfo("FuelLevel: " + StringTools.format(fuelLevel ,"#0.0") + " %");
        }
        if (fuelLevel2 > 0.0) {
        Print.logInfo("FuelLvl#2: " + StringTools.format(fuelLevel2 ,"#0.0") + " %");
        }
        if (servingCell != null) {
    ...
    **/
    public static void configInit() 
    {
        ...
        if (dcsc != null) {
            ...
            TK103_KEEPALIVE_ACK      = dcsc.getStringProperty(Constants.CFG_returnAckOnKeepAlive,TK103_KEEPALIVE_ACK);
