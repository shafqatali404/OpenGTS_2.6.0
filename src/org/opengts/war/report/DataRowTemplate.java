    ...
    */
    ...
    public Object getFieldValue(String key, int rowNdx, ReportData rd, ReportColumn rc, Object obj)
    {
        ...
        if (cdv != null) {
            ...
            } catch (Throwable th) { // NullPointerException, ClassCastException, etc.
        ...
        if (obj instanceof DBRecord) {
            String K = key.startsWith("%")? key.substring(1) : key; // "%licensePlate"
            try {
                DBRecord  dbr = (DBRecord)obj;
                DBField dbFld = dbr.getFactory(true).getField(K);
                if (dbFld != null) {
                    Object val = dbr.getFieldValue(K);
