...
//     -Added support for "Group Detail" reporting output to XLS
//  2012/02/03  Martin D. Flynn
//     -Fixed issue where simple (single device) reports may not be saved to output stream.
//     -CSV output format now supports group detail reports
//  2012/05/27  Martin D. Flynn / Ricardo Trigo
//     -Initial support for PDF file output. (not yet fully supported)
// ----------------------------------------------------------------------------
    ...
    implements ReportPresentation
{
    ...
    public static final String TAG_AccountDescription   = "AccountDescription";
        ...
        throws ReportException
    {
        ...
        switch (rptFormat) {
            ...
            case PDF     :
                return this.writePDF( out, indentLevel, rd);                    // pdf
            case HTML    :
            default:
        ...
        throws ReportException
    {
        ...
        /* report subtitle row */
        {
            ...
            // -- Graph
            if (!isEMail && (ndx < 0) && rd.getSupportsGraphDisplay()) {
                URIArg graphURL = new URIArg(rd.getRefreshURL());
                graphURL.addArg(ReportURL.RPTARG_FORMAT[0],ReportData.FORMAT_GRAPH);
                String graphURLStr = EncodeURL(reqState,graphURL);
                if (!StringTools.isBlank(graphURLStr)) {
                    ...
                    linkSB_R.append("<span class='spanLink' onclick=\"javascript:openResizableWindow('"+graphURLStr+"','ReportGraph',"+sz.getWidth()+","+sz.getHeight()+");\">"+desc+"</span>");
                }
            }
            // -- Map
        ...
        if (this.rptBody.getRecordCount(rd) <= 0) { // rd.getReportRecordCount()
            out.print("<tr>\n");
        ...
        if (this.rptBody.isPartial(rd)) {   // rd.getReportIsPartial()
            out.print("<tr>\n");
        ...
        throws ReportException
    {
        ...
        /* Account Description */
        pw.print(PFX2);
        pw.print(XMLTools.startTAG(isSoapRequest,TAG_AccountDescription,"",false,false)); // TAG_AccountDescription
        pw.print(XmlFilter(isSoapRequest,rd.getAccountDescription()));
        pw.print(XMLTools.endTAG(isSoapRequest,TAG_AccountDescription,true));             // TAG_AccountDescription
        ...
        if (rptFormat.equals(ReportURL.Format.EMAIL)) { // FORMAT_EMAIL
            String rptMsg = "";
            boolean sent  = false;
            ...
            try {
                ...
                if (StringTools.isBlank(toEmail)) {
                    ...
                    String subj = i18n.getString("ReportDisplay.reportTitle","Report")+": "+rd.getReportTitle();
                    StringBuffer body = new StringBuffer();
                    body.append(i18n.getString("ReportDisplay.account","Account")).append(": ");
                        body.append("[").append(rd.getAccountID()).append("] ");
                        body.append(account.getDescription());
                        body.append("\n");
                    body.append(subj);
                        body.append("\n");
                    //body.append(StringTools.trim(emailURL.toString()));
                    //    body.append("\n");
                    ...
                    String smtpHost = (smtpProps != null)? smtpProps.getHost() : "?";
                    SendMail.send(frEmail,toEmail,subj,body.toString(),attach,smtpProps,false);
                    Print.logInfo("Email queued/sent (via "+smtpHost+"): ["+rd.getAccountID()+"] " + toEmail);
                    rptMsg = "EMail queued/sent: " + toEmail;
                    sent   = true; // or queued
                    // -- SystemAudit (reportID)
                    Audit.reportEmail(rd.getAccountID(),rd.getUserID(),reportID, toEmail);
                }
        ...
        pw.print(XmlFilter(isSoapRequest,this.rptBody.isPartial(rd)));  // rd.getReportIsPartial()
    ...
    {
        // -- This EHTML path is chosen when selecting to "EMail" a report from the web-service.
        ...
        try {
            ...
            success = true;
            //if (!success) throw new IOException("hack to get around Java requirements");
        } catch (IOException ioe) {
            success = false;
        } finally {
            if (emailLinkProps != null) {
    ...
    /* see also _writeHTML(...) */
    private int writeXLS(OutputProvider out, int level, ReportData rd, boolean xlsx) // FORMAT_XLS
        throws ReportException
    {
        Print.logInfo("Excel spreadsheet report output (xlsx="+xlsx+") ...");
        ...
        /* no ReportData */
        if (rd == null) {
            Print.logInfo("No ReportData (Skipping Excel spreadsheet output)");
            return 0;
        }
    ...
    private int _writeXLS(ReportSpreadsheet rptSS, int level, ReportData rd, boolean xlsx, int devCount) // FORMAT_XLS
        throws ReportException
    {
