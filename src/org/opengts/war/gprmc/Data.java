...
//     -Added CHECK_AUTH_CODE (defaults to "false")
// ----------------------------------------------------------------------------
    ...
    extends CommonServlet
{
    ...
    /* DCS version */
    public static final String VERSION                      = "2.2.8"; // 2.5.8-B11
    ...
    /* check device auth/pin code */
    private static boolean  CHECK_AUTH_CODE                 = false;
    ...
    public static final String  CONFIG_CHECK_AUTH_CODE      = DEVICE_CODE + ".checkAuthCode";
    ...
    static {
        ...
        CHECK_AUTH_CODE    = RTConfig.getBoolean(CONFIG_CHECK_AUTH_CODE  , CHECK_AUTH_CODE);
        ...
        String charEnc     = StringTools.getCharacterEncoding();

        /* header */
        Data.logInfo("-----------------------------------------------------------------------");
        Data.printHeaderLine("Version"                , VERSION               , null);
        Data.printHeaderLine("Character Encoding"     , charEnc               , null);
        ...
        Data.printHeaderLine("Check Auth Code"        , CHECK_AUTH_CODE+""    , CONFIG_CHECK_AUTH_CODE);
        ...
        Data.printHeaderLine("'OK' Response"          , RESPONSE_OK           , CONFIG_RESPONSE_OK);
        Data.printHeaderLine("'Error' Response"       , RESPONSE_ERROR        , CONFIG_RESPONSE_ERROR);
        Data.printHeaderLine("'Auth' Response"        , RESPONSE_NOT_AUTH     , CONFIG_RESPONSE_NOT_AUTH);
        Data.logInfo("-----------------------------------------------------------------------");
    ...
    **/
        ...
        String authCode)
    {
        ...
        /* validate auth code */
        if (CHECK_AUTH_CODE && !device.validateDataKey(authCode)) {
            // -- 'authCode' does not match device 'dataKey'
            Data.logError(null, "Invalid PIN for device: " + authCode);
            // -- perform other actions here?
            // -    SendMail.send(From, To, Subject, Message, null/*SmtpProps*/);
            // -    SendMail.sendSysadmin(Subject, Message);
            // -    <EventRuleFactory>.executeRule(device,...
            return null;
        }
        ...
        throws ServletException, IOException
    {
        ...
        if ((statusCode != StatusCodes.STATUS_LOCATION) || (eventCount == 0)) {
