    ...
    implements Constants
{
  ...
  //public  static final String COMMAND_INFO_REFRESH        = "refreshList";
    ...
    public  static final String PARM_DEV_FUEL_PROFILE_SCALE = "d_fuelprofScale";
    public  static final String PARM_DEV_FUEL_PROFILE_OFFS  = "d_fuelprofOffs";
  ...
  //public  static final String PARM_WORKORDER_ID           = "d_workid";
    ...
    */
    ...
    private String _updateDeviceTable(RequestProperties reqState)
    {
        ...
        String  fuelProfSca = AttributeTools.getRequestString(request, PARM_DEV_FUEL_PROFILE_SCALE, "");
        String  fuelProfOfs = AttributeTools.getRequestString(request, PARM_DEV_FUEL_PROFILE_OFFS , "");
      ...
      //String  worderID    = AttributeTools.getRequestString(request, PARM_WORKORDER_ID          , "");
        try {
            ...
            if (showFuelProf && editFuelProf) {
                String flpn = fuelProf.equalsIgnoreCase(FuelLevelProfile.FLP_NONE_ID)? "" : fuelProf;
                if ((flpn.indexOf(":") < 0)                                       && 
                    (flpn.equalsIgnoreCase(FuelLevelProfile.FLP_LINEAR_NAME  ) || 
                     flpn.equalsIgnoreCase(FuelLevelProfile.FLP_CYLINDER_NAME)   )  ) {
                    int scale  = StringTools.parseInt(fuelProfSca,0);
                    int offset = StringTools.parseInt(fuelProfOfs,0);
                    if (scale > 0) {
                        flpn += ":" +scale; // "CYLIDER:600"
                        if (offset > 0) {
                            flpn += "," + offset; // "CYLIDER:600,40"
                        }
                    }
                }
                if (!selDev.getFuelTankProfile().equals(flpn)) {
                    selDev.setFuelTankProfile(flpn);
                }
            }
            // -- fuel economy (km/L)
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                    ...
                    if (showFuelProf && _viewFuelPrf) {
                        ...
                        if (StringTools.isBlank(profSel)) { profSel = FuelLevelProfile.FLP_NONE_ID; }
                        switch (fuelProfType) {
                            default:
                            case 0: {// "none"
                                // -- no-op
                            ...
                            case 1: { // "text"
                                // -- view as TextField
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE, _editFuelPrf, i18n.getString("DeviceInfo.fuelProfile","Fuel Tank Profile")+":", profSel, 80, 250, null));
                                } break;
                            case 2: { // "select"
                                // -- view as ComboBox
                            ...
                            case 3: { // "scale"
                                ...
                                String offset;
                                if (profSel == null) {
                                    name   = FuelLevelProfile.FLP_NONE_NAME;
                                    scale  = "";
                                    offset = "";
                                } else
                                if (profSel.indexOf(":") < 0) {
                                    name = profSel.toUpperCase();
                                    if (!name.equals(FuelLevelProfile.FLP_LINEAR_NAME)  &&
                                        !name.equals(FuelLevelProfile.FLP_CYLINDER_NAME)  ) {
                                        name   = FuelLevelProfile.FLP_NONE_NAME;
                                        scale  = "";
                                        offset = "";
                                    } else {
                                    ...
                                    FuelLevelProfile flp = new FuelLevelProfile(profSel, null);
                                    name   = flp.getID();
                                    scale  = String.valueOf(Math.round(flp.getScale()));
                                    offset = String.valueOf(Math.round(flp.getOffset()));
                                } 
                                ...
                                ComboMap flpCombo = new ComboMap(profMap);
                                out.println(FormRow_ComboBox( PARM_DEV_FUEL_PROFILE      , _editFuelPrf, i18n.getString("DeviceInfo.fuelProfile"     ,"Fuel Tank Profile"       )+":", name  , flpCombo, "", -1));
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE_SCALE, _editFuelPrf, i18n.getString("DeviceInfo.fuelProfileScale","Fuel Tank Profile Scale" )+":", scale , 5, 5, null));
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE_OFFS , _editFuelPrf, i18n.getString("DeviceInfo.fuelProfileScale","Fuel Tank Profile Offset")+":", offset, 5, 5, null));
                                } break;
                        }
                    }
                    // -- fuel economy
