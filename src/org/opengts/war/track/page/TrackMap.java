    ...
    implements Constants
{
    ...
    public static final String   PROP_statusMarkers             = "statusMarkers";
    ...
    protected void postInit()
    {
        ...
        if (!StringTools.isBlank(statusCodesCSV)) {
            ...
            for (int i = 0; i < val.length; i++) {
                ...
                //Print.logInfo("Map StatusCodes: 0x" + StringTools.toHexString(this.statusCodes[i],16));
            }
        }

        /* status markers */
        this.statusMarkers = null;
        String statusMarkersCSV = this.getStringProperty(privLabel,PROP_statusMarkers,null);
        if (!StringTools.isBlank(statusMarkersCSV)) {
            String val[] = StringTools.parseArray(statusMarkersCSV);
            this.statusMarkers = new int[val.length];
            for (int i = 0; i < val.length; i++) {
                this.statusMarkers[i] = StringTools.parseInt(val[i], StatusCodes.STATUS_NONE);
                //Print.logInfo("Map StatusMarkers: 0x" + StringTools.toHexString(this.statusMarkers[i],16));
            }
        }
    ...
    /**
    *** Gets the list of status-codes to be displayed on the map.
    **/
    ...
    /**
    *** Gets the list of most-recent marker status-codes.
    **/
    protected int[] getStatusMarkers()
    {
        return this.statusMarkers; // may be null
    }
        ...
        throws IOException
    {
        ...
        if (device == null) {
            ...
            // -- check for true|false|default|icon|percent
            ...
            if (StringTools.isBlank(blvlProp) || (blvlProp.indexOf("false") >= 0)) {
                ...
                if ((blvlProp.indexOf("default") >= 0) || (blvlProp.indexOf("device") >= 0)) {
                    // -- ie. "icon,device": show battery level if device as battery level value
                    showBatteryLevel = (device.getLastBatteryLevel() > 0.0)? dispType : 0;
                } else {
                    // -- ie. "icon": show battery level as specified
        ...
        /* status code/markers */
        reqState.setStatusCodes(this.getStatusCodes());
        reqState.setStatusMarkers(this.getStatusMarkers());
        ...
        if (cmdName.equals(COMMAND_MAP_UPDATE)) {
            ...
            mapProvider.writeMapUpdate(
                EventUtil.MAPDATA_JSON, // MAPDATA_DEFAULT, // [2.5.8-B04]
                reqState); // XML/JSON
            return;
        }
        ...
        if (cmdName.equals(COMMAND_DEVICE_PING)) {
            ...
            if (!isFleet) {
                ...
                String pingName   = (String)AttributeTools.getRequestAttribute(request, PARM_DEVICE_COMMAND, ""); // session or query
                String pingArgs[] = null;
                String groupID    = reqState.getSelectedDeviceGroupID();
                DeviceGroup group = reqState.getSelectedDeviceGroup();
                if (group == null) {
                    Print.logInfo("Group not found: " + groupID);
            ...
            public void write(PrintWriter out) throws IOException {
                ...
                */
                ...
                if (!isFleet && (showBatteryLevel != 0)) {
                    ...
                    double lastBattLvl = device.getLastBatteryLevel();
                    double lastBattPct = (lastBattLvl <= 1.0)? (lastBattLvl * 100.0) : lastBattLvl;
                    int battLvl = (int)Math.round(lastBattPct);
                    String battIcon;
