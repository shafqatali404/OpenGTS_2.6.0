...
//     -Changed Google pushpins "mm_20_COLOR.png" to new location.
// ----------------------------------------------------------------------------
...
public class GoogleKML
{
    ...
    public static String GooglePushpinBase()
    {
        ...
        // GoogleKML.pushpinBaseURL=http://localhost:8080/track/images/google/
        return RTConfig.getString(DBConfig.PROP_GoogleKML_pushpinBaseURL, 
            "http://labs.google.com/ridefinder/images/"             // <-- may no longer be valid
            //"http://maps.gstatic.com/mapfiles/ridefinder-images/" // <-- alternate location
            );
    }
    ...
    private StringBuffer _createPlacemarkKML(StringBuffer sb,
        GeoPoint gp, 
        ...
        String style)
    {
        ...
        /* header */
        String tab = StringTools.replicateString(" ", 4);
        sb.append(tab + "<Placemark>\n");

        /* name/description/style */
        sb.append(tab + "  <name>" + StringTools.trim(name) + "</name>\n");
        sb.append(tab + "  <description><![CDATA[").append(StringTools.trim(desc)).append("]]></description>\n");
        sb.append(tab + "  <styleUrl>#" + StringTools.blankDefault(style,STYLE_DEFAULT) + "</styleUrl>\n");
        ...
        String gpsPoint = lonStr + "," + latStr;
        sb.append(tab + "  <Point><coordinates>" + gpsPoint + "</coordinates></Point>\n");
    ...
    *** @param gpList The collection of GeoPoints
    *** @return The KML String representation
    **/
    public String createKML(Collection<GeoPoint> gpList)
    {
        ...
        /* Route Style */
        sb.append(XML_RouteStyle);

        /* standard styles */
        sb.append(_createStyle(STYLE_DEFAULT      , getGooglePushpin("green")));
        sb.append(_createStyle(STYLE_MOVING       , getGooglePushpin("green")));
        sb.append(_createStyle(STYLE_MOVING_LAST  , getGooglePushpin("green.last")));
        sb.append(_createStyle(STYLE_STOPPED      , getGooglePushpin("red")));
        sb.append(_createStyle(STYLE_STOPPED_LAST , getGooglePushpin("red.last")));
        sb.append(_createStyle(STYLE_SLOW         , getGooglePushpin("yellow")));
        sb.append(_createStyle(STYLE_SLOW_LAST    , getGooglePushpin("yellow.last")));
        ...
        if (!ListTools.isEmpty(gpList)) {
            ...
            for (GeoPoint gp : gpList) {
                String name = "#" + (gpCnt++);
                String desc = name;
                this._createPlacemarkKML(sb, gp, name, desc, null);
            }
        }
    ...
    /**
    *** Writes a marker for the specified EventData record
    **/
