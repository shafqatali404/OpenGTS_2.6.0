    ...
    **/
    public boolean saveEventDataFieldValue(EventData evdb, double value) 
    {
        ...
        if (Double.isNaN(value)) {
            ...
            Print.logWarn("NaN value specified, ignored ...");
            return false;
        }
        ...
        if (this._hasDBFields()) {
            for (DBField dbf : this._getDBFields()) {
                ...
                if (objVal != null) {
                    ...
                    Print.logWarn("Unable to convert value to type: " + StringTools.className(dbf.getTypeClass()));
                    allOk = false;
                }
            }
        } else {
            Print.logWarn("No Analog fields defined");
            allOk = false;
        }
