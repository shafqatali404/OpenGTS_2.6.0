    ...
    extends AccountRecord<Geozone>
{
    ...
    /* supports PointsOfInterest? (not fully implemented) */
    private static final boolean SupportPointsOfInterest    = false;
    ...
    **/
    public static double GetMinimumRadius(int gzType)
    {
        ...
        if (gzType == GeozoneType.BOUNDED_RECT.getIntValue()) {
          //return RTConfig.getDouble(DBConfig.PROP_Geozone_minRadius_boundedRectangle, 0.0);
            return 0.0;
        } else
        ...
        if (gzType == GeozoneType.POINT_OF_INTEREST.getIntValue()) {
          //return RTConfig.getDouble(DBConfig.PROP_Geozone_minRadius_pointOfInterest, MIN_RADIUS_METERS);
            return 0.0;
        } else {
            return RTConfig.getDouble(DBConfig.PROP_Geozone_minRadius_pointRadius, MIN_RADIUS_METERS);
        }
    ...
    **/
    public static double GetMinimumRadius()
    {
      ...
      //minRad = Math.min(minRad, GetMinimumRadius(GeozoneType.BOUNDED_RECT      .getIntValue()));
      ...
      //minRad = Math.min(minRad, GetMinimumRadius(GeozoneType.POINT_OF_INTEREST .getIntValue()));
    ...
    **/
    public static double GetMaximumRadius(int gzType)
    {
        ...
        if (gzType == GeozoneType.BOUNDED_RECT.getIntValue()) {
          //return RTConfig.getDouble(DBConfig.PROP_Geozone_maxRadius_boundedRectangle, 0.0);
            return 0.0;
        } else
        if (gzType == GeozoneType.SWEPT_POINT_RADIUS.getIntValue()) {
        ...
        if (gzType == GeozoneType.POINT_OF_INTEREST.getIntValue()) {
          //return RTConfig.getDouble(DBConfig.PROP_Geozone_maxRadius_pointOfInterest, MAX_RADIUS_METERS);
            return 0.0;
        } else {
            return RTConfig.getDouble(DBConfig.PROP_Geozone_maxRadius_pointRadius, MAX_RADIUS_METERS);
        }
    ...
    **/
    public static double GetMaximumRadius()
    {
      ...
      //maxRad = Math.max(maxRad, GetMaximumRadius(GeozoneType.BOUNDED_RECT      .getIntValue()));
      ...
      //maxRad = Math.max(maxRad, GetMaximumRadius(GeozoneType.POINT_OF_INTEREST .getIntValue()));
    ...
    //    geofence is a match.
    // GeozoneType.SWEPT_POINT_RADIUS: A circle of 'radius' meters is swept between all available points.  
    //    If the target point is within the area swept out by this circle (ie. the "geocorridor"),  
    //    then this geofence is a match.
    // GeozoneType.POLYGON: For this type of geofence, all points represent a vertice in a polygon.   
    //    If the target point is within this polygon, then this geofence is a match.
    // GeozoneType.POI(pending): For this type of geofence, only the first GeoPoint is used.
    //    Since there is no radius, this type of geofence cannot be used for arrive/depart/etc.
    ...
    public enum GeozoneType implements EnumTools.StringLocale, EnumTools.IntValue {
        ...
        POINT_OF_INTEREST   (4, I18N.getString(Geozone.class,"Geozone.type.pointOfInterest" , "PointOfInterest" ), false);
        private int         vv = 0;
    ...
    {
        if (z != null) {
            int gzType = z.getZoneType();
            return (gzType == GeozoneType.POINT_OF_INTEREST.getIntValue())? true : false;
        } else {
            return false;
        }
    }
    ...
    **/
    ...
    public static final String FLD_arrivalStatusCode        = "arrivalStatusCode";
    public static final String FLD_departureZone            = "departureZone";
    public static final String FLD_departureStatusCode      = "departureStatusCode";
    ...
    public static final String FLD_iconName                 = "iconName";       // pushpinID
    // -- geozone definition
    ...
    private static DBField FieldInfo[] = {
        ...
        new DBField(FLD_arrivalStatusCode   , Integer.TYPE  , DBField.TYPE_UINT32      , "Arrival Code"     , "edit=2 editor=statusCode format=X2"), //export=true
        new DBField(FLD_departureZone       , Boolean.TYPE  , DBField.TYPE_BOOLEAN     , "Departure Zone"   , "edit=2 export=true"),
        new DBField(FLD_departureStatusCode , Integer.TYPE  , DBField.TYPE_UINT32      , "Departure Code"   , "edit=2 editor=statusCode format=X2"), //export=true
        ...
        new DBField(FLD_iconName            , String.class  , DBField.TYPE_STRING(24)  , "Pushpin ID"       , "edit=2 export=true"),
        // -- geozone definition
    ...
    public static final DBField PriorityFieldInfo[] = {
        // Overlap Priority
        new DBField(FLD_priority            , Integer.TYPE  , DBField.TYPE_UINT32      , "Overlap Priority"     , "edit=2 altkey=priority export=true"),
        // In-zone speed limit
    ...
    **/
    ...
    {
        if (!Geozone.isPointOfInterest(this)) {
    ...
    **/
    public void setPriority(int v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_priority, 0);
        }
    }
    ...
    **/
    public void setSpeedLimitKPH(double v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_speedLimitKPH, 0.0);
        }
    }
    ...
    **/
    ...
    {
        return this.getZoneTypeDescription(Account.GetLocale(this.getAccount()));
    }
    ...
    public void setDefaultRadius()
    {
        ...
        if (gzType == GeozoneType.BOUNDED_RECT.getIntValue()) {
            int radM = 0;
            this.setRadius(radM);    // this is ignored for bounded-rectangles anyway
        } else
        if (gzType == GeozoneType.SWEPT_POINT_RADIUS.getIntValue()) {
        ...
        if (gzType == GeozoneType.POINT_OF_INTEREST.getIntValue()) {
            int radM = 0;
            this.setRadius(radM);    // this is ignored for POI anyway
        } else {
            int radM = RTConfig.getInt(DBConfig.PROP_Geozone_dftRadius_pointRadius, 3000);
            this.setRadius(radM);
        }
    ...
    /* gets the pushpin ID that respresents this geozone */
    ...
    /* sets the pushpin ID that respresents this geozone */
    ...
    *** @param iconKeys The icon pushpinID map
    *** @param dft      The default pushpin index if the pushpinID is not found or blank
    *** @return The pushpin index, or 'dft' if not found
    **/
    public int getPushpinIconIndex(OrderedSet<String> iconKeys, int dft)
    {
        return EventData._getPushpinIconIndex(this.getIconName(),iconKeys,dft);
    }

    /**
    *** Gets the pushpin icon index for the specified pushpin id
    *** @param iconKeys The icon pushpinID map
    *** @return The pushpin index, or -1 if not found
    **/
    public int getPushpinIconIndex(OrderedSet<String> iconKeys)
    {
        return this.getPushpinIconIndex(iconKeys,-1);
    }
    ...
    private static int _GetGeoPointFieldCount() // was "GetGeoPointCount()"
    {
    ...
    /* get the first GeoPoint */
    public GeoPoint getFirstGeoPoint()
    {
        return this.getGeoPointAt(0,null);
    }
    ...
    public void setClientID(int v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_clientID, 0);
        }
    }
    ...
    **/
    ...
    /**
    *** Returns an array of active Geozones found in the specified lizt that are applicable to the
    *** specified DeviceID.  
    *** @since 2.6.0-B47
    *** @param gza       The list of Geozones
    *** @param deviceID  The device id.
    *** @return The array of Geozones that are applicable to the specified DeviceID
    **/
    public static Geozone[] getGeozonesForDevice(Geozone gza[], String deviceID)
    {
        if (!ListTools.isEmpty(gza)) {
            Vector<Geozone> gzList = null;
            for (Geozone gz : gza) {
                if ((gz != null) && gz.isDeviceInGroup(deviceID)) {
                    if (gzList == null) { gzList = new Vector<Geozone>(); }
                    gzList.add(gz);
                }
            }
            return !ListTools.isEmpty(gzList)? gzList.toArray(new Geozone[gzList.size()]) : null;
        }
        return null;
    }
    ...
    /** 
    *** Returns the first (highest priority) Geozone in which the point resides that is 
    *** also applicable for the specified DeviceID.
    *** @since 2.4.9-B15
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* first get all active Geozones containing point (sorted by highest-to-loweest priority) */
        Geozone gzAll[] = Geozone.getGeozones(acctID, gp); // all active Geozones containing point
        if (ListTools.isEmpty(gzAll)) {
            // -- no active geozones found
            return null;
        }

        /* extract first active Geozone for Device */
        Geozone gz = Geozone.getFirstGeozoneForDevice(gzAll, devID);
        if (gz == null) {
    ...
    /** 
    *** Returns a map of the first (highest priority) Geozone in each arrive/depart category, in which 
    *** the point resides that is also applicable for the specified DeviceID.
    *** @since 2.6.0-B47
    *** @param acctID The AccountID
    *** @param gp     The GeoPoint
    *** @param devID  The DeviceID
    *** @return An array of Geozones
    **/
    public static Map<String,Geozone> getGeozonesForDevice(String acctID, GeoPoint gp, String devID)
        throws DBException
    {
        ...
        /* extract Geozone for Device */
        Geozone gza[] = Geozone.getGeozonesForDevice(gzAll, devID);
        if (ListTools.isEmpty(gza)) {
            // -- no Geozones apply to this device
            Print.logDebug("Geozones found @ "+gp+", but none apply to Device: " + acctID+"/"+devID);
            return null;
        }

        /* get the first geozone for each status code category */
        Map<String,Geozone> gzCatMap = null;
        for (Geozone gz : gza) {
            String cat = gz.getArrivalStatusCodeCategory(); // gz.getDepartureStatusCodeCategory();
            if (StringTools.isBlank(cat) || cat.equalsIgnoreCase("NONE")) {
                // -- not a paired status code category
                Print.logError("Geozone specifies an invalid Arrival status-code: " + gz.getGeozoneID());
            } else
            if ((gzCatMap == null) || !gzCatMap.containsKey(cat)) {
                // -- new category, add Geozone
                if (gzCatMap == null) { gzCatMap  = new HashMap<String,Geozone>(); }
                gzCatMap.put(cat,gz); // category
            }
        }
        return gzCatMap; // may be null
    ...
    **/
    public void setReverseGeocode(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_reverseGeocode, false);
        }
    }
    ...
    **/
    public void setArrivalZone(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_arrivalZone, false);
        }
    }
    ...
    **/
    public int getArrivalStatusCode()
    {
        ...
        // -   *STATUS_GEOFENCE_ACTIVE / STATUS_GEOFENCE_INACTIVE
        // -   *STATUS_GEOBOUNDS_ENTER / STATUS_GEOBOUNDS_EXIT
        // -    STATUS_CORRIDOR_ARRIVE / STATUS_CORRIDOR_DEPART
        // -    STATUS_CORRIDOR_ACTIVE / STATUS_CORRIDOR_INACTIVE
        // -    STATUS_PERSON_ENTER    / STATUS_PERSON_EXIT
        // -   *STATUS_INTRUSION_ON    / STATUS_INTRUSION_OFF
        // -    STATUS_BREACH_ON       / STATUS_BREACH_OFF
        // -   *STATUS_TRACK_START     / STATUS_TRACK_STOP
        // -   *STATUS_TRACK_STOP      / STATUS_TRACK_START
        // -   *STATUS_JOB_START       / STATUS_JOB_STOP
        // -    STATUS_JOB_STOP        / STATUS_JOB_START
        // -    STATUS_DUTY_ON         / STATUS_DUTY_OFF
        // -    STATUS_DUTY_OFF        / STATUS_DUTY_ON
        if (!Geozone.isPointOfInterest(this) && this.getArrivalZone()) {
            Integer v = (Integer)this.getFieldValue(FLD_arrivalStatusCode);
            int sc = (v != null)? v.intValue() : StatusCodes.STATUS_NONE;
            return (sc > 0)? sc : StatusCodes.STATUS_GEOFENCE_ARRIVE;
        } else {
            return StatusCodes.STATUS_GEOFENCE_ARRIVE; // StatusCodes.STATUS_NONE
        }
    }
    ...
    **/
    ...
    {
        if (!Geozone.isPointOfInterest(this)) {
            int sc = (v > 0)? v : StatusCodes.STATUS_NONE;
            this.setFieldValue(FLD_arrivalStatusCode, sc);
        } else {
            this.setFieldValue(FLD_arrivalStatusCode, StatusCodes.STATUS_NONE);
        }
    }

    /**
    *** Gets the arrival status code category
    **/
    public String getArrivalStatusCodeCategory()
    {
        int sc = this.getArrivalStatusCode();
        return StatusCodes.GetPairedStatusCodesCategory(sc); // first category
    }
    ...
    **/
    public void setDepartureZone(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_departureZone, false);
        }
    }
    ...
    **/
    ...
    {
        if (!Geozone.isPointOfInterest(this) && this.getDepartureZone()) {
            Integer v = (Integer)this.getFieldValue(FLD_departureStatusCode);
            int sc = (v != null)? v.intValue() : StatusCodes.STATUS_NONE;
            return (sc > 0)? sc : StatusCodes.STATUS_GEOFENCE_DEPART;
        } else {
            return StatusCodes.STATUS_GEOFENCE_DEPART; // StatusCodes.STATUS_NONE
        }
    }
    ...
    **/
    public void setDepartureStatusCode(int v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            int sc = (v > 0)? v : StatusCodes.STATUS_NONE;
            this.setFieldValue(FLD_departureStatusCode, sc);
        } else {
            this.setFieldValue(FLD_departureStatusCode, StatusCodes.STATUS_NONE);
        }
    }

    /**
    *** Gets the departure status code category
    **/
    public String getDepartureStatusCodeCategory()
    {
        int sc = this.getDepartureStatusCode();
        return StatusCodes.GetPairedStatusCodesCategory(sc); // first category
    }
    ...
    **/
    public void setAutoNotify(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_autoNotify, false);
        }
    }
    ...
    **/
    public void setZoomRegion(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_zoomRegion, false);
        }
    }
    ...
    **/
    public void setCorridorID(String v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_corridorID, "");
        }
    }
    ...
    **/
    ...
    public void setCorrStartSelector(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_corrStartSelector, "");
        }
    }
    ...
    public void setCorrEndSelector(boolean v)
    {
        if (!Geozone.isPointOfInterest(this)) {
            ...
            this.setFieldValue(FLD_corrEndSelector, "");
        }
    }
    ...
    private static GeozoneChecker geozonePointOfInterest  = null;
    ...
    private static void _initZoneTypes()
    {
        ...
        /* GeozoneType.POINT_OF_INTEREST: (pending - not fully implemented) */
        if (SupportPointsOfInterest) {
            geozonePointOfInterest = new GeozoneChecker() {
                public boolean containsPoint(GeoPoint gpTest, GeoPoint gpList[], double radiusKM) {
                    return false; // no radius, always false
                }
            };
    ...
    public static boolean IsGeozoneTypeSupported(GeozoneType type)
    {
        switch (type) {
            ...
            case POINT_OF_INTEREST   : return (geozonePointOfInterest  != null);
        }
        return false;
    }
    ...
    public boolean containsPoint(GeoPoint gp)
    {
        ...
        switch (Geozone.getGeozoneType(this)) {
            ...
            case POINT_OF_INTEREST:  {
                double radiusKM  = this.getRadiusKilometers(); // should be "0.0"
                if (geozonePointOfInterest != null) {
                    return geozonePointOfInterest.containsPoint(gp,gzPts,radiusKM);
                } else {
                    return false;
                }
            }
    ...
    **/
    public static GeoBounds getBoundingBox(GeozoneType zoneType, GeoPoint gp[], double radiusM)
    {
        ...
        switch (zoneType) {
            ...
            case POINT_OF_INTEREST: {
                // -- bounded by single point
                if (ListTools.size(gp) >= 1) {
                    bounds.extendByPoint(gp[0]);
                }
            } break;
        }
        return bounds;
    ...
    public GeoBounds getBoundingBox()
    {
        ...
        switch (type) {
            ...
            case POLYGON           :
                ...
                break;
            case POINT_OF_INTEREST :
                return Geozone.getBoundingBox(type,gp,0.0); // will take only first point
        }
    ...
    **/
        ...
        boolean reverseGeocodeOnly)
    {
        ...
        /* Account */
        dwh.append(dwh.EQ(Geozone.FLD_accountID,acctId));

        /* specific zoneID */
        if (!StringTools.isBlank(zoneId)) {
            dwh.append(dwh.AND_(dwh.EQ(Geozone.FLD_geozoneID,zoneId)));
            // specific sortID
            if (sortId >= 0) {
                dwh.append(dwh.AND_(dwh.EQ(Geozone.FLD_sortID,sortId)));
            }
        }

        /* active geozones only? */
        if (activeOnly && !Geozone.IsGlobalActive()) {
            dwh.append(dwh.AND_(dwh.NE(Geozone.FLD_isActive,0)));
        }

        /* client upload geozones only? */
        if (clientUploadOnly) {
            dwh.append(dwh.AND_(dwh.NE(Geozone.FLD_clientUpload,0)));
          //dwh.append(dwh.AND_(dwh.GT(FLD_clientID,0)));
        }

        /* reverseGeocodeOnly geozones only? */
        if (reverseGeocodeOnly) {
            dwh.append(dwh.AND_(dwh.NE(Geozone.FLD_reverseGeocode,0)));
        }
        ...
        if ((bounds != null) && bounds.isValid()) {
            ...
            if (USE_BOUNDING_BOX) {
                ...
                dwh.append(dwh.AND_(
                    dwh.AND(
                        dwh.LE(Geozone.FLD_minLatitude , maxLat),
                        dwh.GE(Geozone.FLD_maxLatitude , minLat),
                        dwh.LE(Geozone.FLD_minLongitude, maxLon),
                        dwh.GE(Geozone.FLD_maxLongitude, minLon)
                    )
                ));
            } else {
                dwh.append(dwh.AND_(
                    dwh.AND(
                        dwh.LE(Geozone.FLD_latitude1 , maxLat),
                        dwh.GE(Geozone.FLD_latitude1 , minLat),
                        dwh.LE(Geozone.FLD_longitude1, maxLon),
                        dwh.GE(Geozone.FLD_longitude1, minLon)
                    )
                ));
            }
        } else
        if (centerGP != null) {
            ...
            if (USE_BOUNDING_BOX) {
                dwh.append(dwh.AND_(
                    dwh.AND(
                        dwh.LE(Geozone.FLD_minLatitude , lat),
                        dwh.GE(Geozone.FLD_maxLatitude , lat),
                        dwh.LE(Geozone.FLD_minLongitude, lon),
                        dwh.GE(Geozone.FLD_maxLongitude, lon)
                    )
                ));
            } else {
                ...
                dwh.append(dwh.AND_(
                    dwh.AND(
                        dwh.LE(Geozone.FLD_latitude1 , lat + ofsGP.getOffsetLatitude() ),
                        dwh.GE(Geozone.FLD_latitude1 , lat - ofsGP.getOffsetLatitude() ),
                        dwh.LE(Geozone.FLD_longitude1, lon + ofsGP.getOffsetLongitude()),
                        dwh.GE(Geozone.FLD_longitude1, lon - ofsGP.getOffsetLongitude())
                    )
                ));
            }
        }
    ...
    {
        Geozone gz = Geozone.getGeozone(acctID, null/*ZoneID*/, gp, true/*RG*/);
    ...
    public static Geozone getGeozone( // active only
    ...
    /* Get all active Geozones in which the GeoPoint resides (sorted by priority) */
        ...
        throws DBException
    {
        ...
        /* where clause */
        // DBSelect: [SELECT] WHERE <Where> ORDER BY geozoneID,priority,sortID
        DBSelect<Geozone> dsel = new DBSelect<Geozone>(Geozone.getFactory());
        dsel.setWhere(Geozone.getWhereClause(
            acctID,                     // accountID
            null,                       // geozoneID
        ...
        Geozone gz[] = null;
        try {
            DBProvider.lockTables(new String[] { TABLE_NAME() }, null); // <-- not needed?
            gz = DBRecord.select(dsel); // select:DBSelect
        } catch (DBException dbe) {
            Print.logError("Geozone error: " + dbe);
            throw dbe;
    ...
    /* Get all active Geozones which touch the specified GeoBounds */
    ...
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(ARG_GEOPOINT)) {
            ...
            try {
                Geozone gz[] = Geozone.getGeozones(acctID, gp); // active only
            ...
            try {
                if (fact.tableExists()) {
                    ...
                    fact.loadTable(loadFile, new ZoneLoadValidator(acctID,validateOnly),
                        !validateOnly/*insertRecord*/,true/*overwriteExisting*/,false/*noDropWarning*/);
