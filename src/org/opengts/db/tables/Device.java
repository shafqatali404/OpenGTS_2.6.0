    ...
    implements DataTransport
{
    ...
    /* deferred rule check */
    public  static      boolean DEFERRED_RULE_CHECK                 = false; // implementation incomplete
    // -- intended to be used for deferred rule checking (a snapshot of the Device record
    // -  is saved and passed to the rule trigger handling).  (Rule "getPreviousGeoPoint" still
    // -  needs some additional implementation before this can be considered complete).
    ...
    **/
    ...
    public static final String FLD_fuelTankProfile2      = "fuelTankProfile2";      // fuel tank profile ID
  ...
  //public static final String FLD_maintDescriptionHR0   = "maintDescriptionHR0";   // hours maint #0 description
    ...
    // -  "vehicleColor", "vehicleWeight", "wheelchairAccess", ...
    ...
    **/
    ...
    {
        Integer v = (Integer)this.getFieldValue(FLD_vehicleYear);
    ...
    **/
    ...
    //  - CYLINDER
    // TODO:
    //  - PROFILE:1.0,0.0:0,0|25,25|50,50|75,75|100,100
    ...
    *** @return True if a Fuel tank profile is defined
    **/
    public boolean hasFuelTankProfile()
    {
        String flpn = this.getFuelTankProfile();
        if (FuelLevelProfile.IsNone(flpn)) {
            // -- device FuelLevelProfile is blank/NONE
            return false;
        } else
        if (this.getFuelLevelProfile(null) == null) {
    ...
    **/
    ...
    /**
    *** Gets the FuelLevelProfile instance.  Returns "LINEAR" if no FuelLevelProfile defined. 
    *** (does not return null)
    **/
    ...
    **/
    ...
    {
        FuelLevelProfile flp = this.getFuelLevelProfile(); // never null
    ...
    *** @return True if a Fuel tank profile is defined
    **/
    public boolean hasFuelTankProfile2()
    {
        String flpn = this.getFuelTankProfile2();
        if (FuelLevelProfile.IsNone(flpn)) {
            // -- device FuelLevelProfile is blank/NONE
            return false;
        } else
        if (this.getFuelLevelProfile2(null) == null) {
            // -- device specified FuelLevelProfile name is not found
            return false;
        } else {
            // -- device specified FuelLevelProfile exists
    ...
    *** Gets the FuelLevelProfile instance.  Returns "dft" if no FuelLevelProfile defined.
    **/
    public FuelLevelProfile getFuelLevelProfile2(FuelLevelProfile dft)
    {
        String profName = this.getFuelTankProfile2();
        return FuelLevelProfile.GetFuelLevelProfile(profName,dft); // may return null, if "dft" is null
    }
    ...
    *** (does not return null)
    **/
    public FuelLevelProfile getFuelLevelProfile2()
    {
        String profName = this.getFuelTankProfile2();
        return FuelLevelProfile.GetFuelLevelProfile(profName); // never null
    }
    ...
    *** Gets the FuelLevelProfile instance
    **/
    public double getActualFuelLevel2(double fuelLevel)
    {
        FuelLevelProfile flp = this.getFuelLevelProfile2(); // never null
        return flp.getActualFuelLevel(fuelLevel);
    }
    ...
    **/
    ...
    {
        // -- used for deferred rule checking (see DEFERRED_RULE_CHECK)
    ...
    **/
    public void log_EventData(int logLevel, EventData ev)
    {
        ...
        sb.append("Event: ");
        if (ev != null) {
            // -- Event: 2015/02/27|12:32:45|PST|1425069165, account|device, 0xF020|Location|geozone, latitude|longitude, speed|heading
            ...
            /* account|device */
            sb.append(ev.getAccountID()).append("|").append(ev.getDeviceID());
            sb.append(", ");
    ...
    **/
    protected boolean _insertEventData(final EventData evdb)
    {
        ...
        // -  2) This Device record at this point _must_ contain old/last field values for proper rule triggers, etc.
        Account account = this.getAccount();
        String  acctID  = this.getAccountID();
        String  devID   = this.getDeviceID();
        ...
        if (DEFERRED_RULE_CHECK) {
            ...
            try {
                ...
                evdb.setDevice(devCopy); // pre-rule-trigger
            } catch (DBException dbe) {
                // -- will not occur
        ...
        */
        ...
        // Note: if DEFERRED_RULE_CHECK==true, then the copy held by evdb.getDevice() is unchanged
        ...
        if (evdb.isValidGeoPoint()) {
            ...
            this.saveOriginalLastValidGeoPoint(); // may be used by DEFERRED_RULE_CHECK
        ...
        /* callback: event has been inserted */
    ...
    **/
    private void _postEventInsertionProcessing(EventData evdb, int extUpdate)
    {
        ...
        /* rule check */
        // -- Cannot generally defer rule check to here!!!
        ...
        if (DEFERRED_RULE_CHECK) {
            // -- assuming that the Device instance in this EventData record is the unchanged-version
            this.checkEventRules(evdb);
        }
        ...
        public void addDeltaStoppedSeconds(long lastTimestamp, long currTimestamp) {
            if (lastTimestamp < currTimestamp) {
                ...
                if (currTimestamp < this.timeStart) {
                    // -- deltaStopSec is before beginning of test-range, ignore
                    deltaStopSec = 0L;
                } else
                if (lastTimestamp > this.timeEnd) {
                    // -- deltaStopSec is after end of test-range, ignore
                    deltaStopSec = 0L;
                } else
    ...
    **/
        ...
        throws DBException
    {
        ...
        for (StoppedSeconds ss : stoppedSec) {
            if ((timeStart <= 0L) || (ss.getTimeStart() < timeStart)) {
                timeStart = ss.getTimeStart();
            }
            if ((timeEnd   <= 0L) || (ss.getTimeEnd()   > timeEnd  )) {
                timeEnd = ss.getTimeEnd();
            }
        }

        /* accumulators */
        final AccumulatorLong    lastEventTime  = new AccumulatorLong(0L);
        final AccumulatorBoolean wasMoving      = new AccumulatorBoolean(true);
        ...
        /* iterate through events and accumulate stop time */
        ...
        EventData.getRangeEvents(
            ...
            new DBRecordHandler<EventData>() {
                public int handleDBRecord(EventData ev) throws DBException {
                    ...
                    boolean isMoving = ((ev.getSpeedKPH() > 0.0) || (ignState == 1))? true : false;
                    // -- 
                    if (!validGPS) {
                    ...
                    if (firstEvent) {
                        ...
                        if (isMoving) {
                            ...
                            // -- start out stopped, start "stopped-timer"
                    ...
                    if (wasMoving.get()) {
                        ...
                        if (isMoving) {
                            ...
                            // -- was moving, now stopped, start timer
                        ...
                        if (isMoving) {
                            // -- was stopped, now moving, tabulate previous stopped time
                            long lastStopTS = lastStopTime.get();
                            long currEvTS   = ev.getTimestamp();
                            for (StoppedSeconds ss : stoppedSec) {
                                ss.addDeltaStoppedSeconds(lastStopTS, currEvTS);
                            }
        ...
        /* final stopped seconds */
        if (!wasMoving.get()) {
            // -- ended with vehicle stopped, add final stopped seconds up to timeEnd
            long lastEventTS = lastEventTime.get();
            for (StoppedSeconds ss : stoppedSec) {
                ss.addDeltaStoppedSeconds(lastEventTS, ss.getTimeEnd());
            }
        }
    ...
    **/
    public boolean isExpectedStoppedSeconds(StoppedSeconds stoppedSec[])
        throws DBException
    {
        int i = this.getFailedExpectedStoppedSecondsIndex(stoppedSec);
    ...
    **/
    public int getFailedExpectedStoppedSecondsIndex(StoppedSeconds stoppedSec[])
        throws DBException
    {
        this.calculateStoppedSecondsInRange(stoppedSec);
        if (ListTools.size(stoppedSec) > 0) {
    ...
    **/
        ...
        throws DBException
    {
        ...
        this.calculateStoppedSecondsInRange(SS);
    ...
    *** Note: "evdb.getDevice()" may be a copy of "this" Device instance.
    *** No adjustments have been made to the Device instance.
    **/
    protected void eventWillInsert(EventData evdb)
    {
        // -- TODO:
        // -  add EventData-will-insert implementation
    }

    /**
    *** Callback indicating EventData instance has been inserted
    *** Note: "evdb.getDevice()" may be a copy of "this" Device instance, but
    *** made prior to any adjustments made from the EventData instance.
    **/
    protected void eventDidInsert(EventData evdb)
    {
        // -- TODO:
        // -  add EventData-did-insert implementation
    }
  ...
  //private static final String ARG_SETPROP[]           = new String[] { "setprop"                  };
    ...
    **/
    public static void main(String args[])
    {
        ...
        /* send command */
        // org.opengts.db.tables.Device -account=ACCOUNT -device=DEVICE -sendCmd=COMMAND_ID
