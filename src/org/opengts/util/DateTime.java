...
//     - fixed "getMonthIndex0" to parse and return proper 0-based index [2.6.0-B42]
// ----------------------------------------------------------------------------
    ...
    implements Comparable, Cloneable
{
    ...
    public static final TimeZone PST                        = DateTime.getTimeZone("US/Pacific");
    ...
    **/
    public static long DaySeconds(int days)
    {
        return (long)days * SECONDS_PER_DAY;
    }
    ...
    *** @return The number of seconds
    **/
    public static long HourSeconds(int hours)
    {
        return (long)hours * SECONDS_PER_HOUR;
    }
    ...
    *** @return The number of seconds
    **/
    public static long HourSeconds(double hours)
    {
        return Math.round(hours * (double)SECONDS_PER_HOUR);
    }
    ...
    *** @return The number of seconds
    **/
    public static long MinuteSeconds(int minutes)
    {
        return (long)minutes * SECONDS_PER_MINUTE;
    }
    ...
    *** @return The number of seconds
    **/
    public static long MinuteSeconds(double minutes)
    {
        return Math.round(minutes * (double)SECONDS_PER_MINUTE);
    }

    // ------------------------------------------------------------------------

    /** 
    *** Returns the MinuteOfDay value of the specified argument, in minutes.<br>
    *** @param arg  The argument for which a MinuteOfDay value is returned.  
    ***             May specify one of the following: 
    ***                 1) A Number instance containing either a HHMM time format
    ***                    or an Epoch timestamp.
    ***                 2) A String containing either "HH:MM" or "HHMM" time format.
    ***                 3) A DateTime instance.
    *** @param tmz  The timezone (only used if <code>arg</code> specifies an Epoch
    ***             time, or a DateTime instance
    *** @return The MinuteOfDay value, or -1 if unable to determine the minute-of-day.
    **/
    public static int MinuteOfDay(Object arg, TimeZone tmz)
    {
        if (arg == null) {
            return -1;
        } else
        if (arg instanceof Number) { // 0830
            long nv = ((Number)arg).longValue();
            if (nv < 0L) {
                return 0;
            } else
            if (nv <= 2400L) {
                // -- 0830, 1730
                int v   = (int)nv;
                int mod = (((v / 100) % 24) * 60) + ((v % 100) % 60);
                return mod;
            } else {
                // -- Epoch?
                DateTime dt = new DateTime(nv, tmz);
                int mod = (dt.getHour24() * 60) + dt.getMinute();
                return mod;
            }
        } else
        if (arg instanceof String) {
            String HHMM = (String)arg;
            int p = HHMM.indexOf(":");
            if (p > 0) {
                // -- "08:30", "17:30"
                int hh  = StringTools.parseInt(HHMM.substring(0,p),0) % 24;
                int mm  = StringTools.parseInt(HHMM.substring(p+1),0) % 60;
                int mod = (hh * 60) + mm;
                return mod;
            } else {
                // -- "0830", "1730"
                int v   = StringTools.parseInt(HHMM,0);
                int mod = (((v / 100) % 24) * 60) + ((v % 100) % 60);
                return mod;
            }
        } else
        if (arg instanceof DateTime) {
            return ((DateTime)arg).getMinuteOfDay(tmz);
        } else {
            return -1;
        }
    }
  ...
  //public static final int  MAY                = MAY;
    ...
    *** @return The default TimeZone (does not return null)
    **/
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (!StringTools.isBlank(plusStr) && StringTools.isLong(plusStr,false)) {
            ...
            if (StringTools.endsWithIgnoreCase(plusStr,"y")) {
                // plus offset Years (approximate)
                ofs = (long)((double)ofs * (double)DateTime.DaySeconds(1) * 365.2425);
            } else
